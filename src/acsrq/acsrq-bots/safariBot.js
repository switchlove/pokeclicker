...
// Cap on consecutive Rock throws used to farm XP from an encounter the bot
// has decided not to catch, before giving up and running.
safariBot.maxRockThrows = 3;
...
function safariBot() {
    ...
    if (Safari.inProgress() && safariBot.stopConditionMet()) {
        clickEngagedS = false;
        Settings.setSettingByName('botstate.safari', false);
        notifyBotComplete('Safari');
        return;
    }
    ...
    // Battle algorithm
        ...
        } else if (Safari.safariLevel() < Safari.maxSafariLevel && SafariBattle.escapeAttempts < safariBot.maxRockThrows) {
            // Not catching this one - squeeze some free Safari XP out of the
            // encounter by throwing Rocks (worth as much XP as a Ball throw)
            // before letting it go, while there's still level left to gain.
            SafariBattle.escapeAttempts++;
...
//Whether the configured safariOpts stop-condition has actually been met (as opposed to
//isRunning's transient "balls empty"/"battle busy"/"modal closed" false cases)
safariBot.stopConditionMet = () => {
    switch (Settings.getSetting('safariOpts').observableValue()) {
        case 'safariOptSC':
            return Safari.completed(true);
        case 'safariOptLvl':
            return Safari.safariLevel() >= Settings.getSetting('maxSafariLvl').value;
        default:
            return false;
    }
};
