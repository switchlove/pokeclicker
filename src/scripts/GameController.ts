...
class GameController {
    ...
    static addKeyListeners() {
        ...
        $shipModal.on('hide.bs.modal', _ => $shipModal.data('disable-toggle', true));
        $shipModal.on('hidden.bs.modal shown.bs.modal', _ => $shipModal.data('disable-toggle', false));
        // Modal Collapse
        ...
        $(document).on('keydown', e => {
            ...
            switch (key) {
                ...
                case Settings.getSetting('hotkey.shop').value:
                    ...
                    break;
                case Settings.getSetting('hotkey.ship').value:
                    // Open the Dock
                    if (player.highestRegion() > 0 && !$shipModal.data('disable-toggle')) {
                        $('.modal').modal('hide');
