...
srBot.trade = function () {
    ...
    const items = shops.map(shop => BerryDeal.list[shop.location]().filter(i => i instanceof BerryDeal && i.item.itemType instanceof PokemonItem && !App.game.party.alreadyCaughtPokemonByName(i.item.itemType.name, true)));
    for (let sdx = 0; sdx < shops.length; sdx++) {
        ShopHandler.showShop(shops[sdx]);
...
srBot.fos = function () {
    ...
    const fossils = UndergroundItems.list.filter(i => i.valueType == UndergroundItemValueType.Fossil && option.test(i.name) && player.itemList[i.itemName]() > 0);

    let smnUsed = false;
    let smnName;
    for (let idx = 0; idx < fossils.length && !smnUsed; idx++) {
        const deal = findFossilRevivalDeal(fossils[idx].itemName);
        const pokemonProfit = deal && GenericDeal.list[deal.traderID]?.()[deal.index]?.profits.find(p => p.type == DealCostOrProfitType.Item);
        if (!pokemonProfit || App.game.party.alreadyCaughtPokemonByName(pokemonProfit.item.name, true)) {
            continue;
        }

        if (GenericDeal.canUse(deal.traderID, deal.index)) {
            GenericDeal.use(deal.traderID, deal.index);
            smnName = pokemonProfit.item.name;
            smnUsed = true;
        }
    }
...
srBot.egg = function () {
    if (App.game.breeding.eggList[0]().type < 0) {
        const region = App.game.challenges.list.regionalAttackDebuff.active() ? Settings.getSetting('breedingRegionalAttackDebuffSetting').value : -1;
        const filteredEggList = [...App.game.party.caughtPokemon]
            .sort(PartyController.compareBy(Settings.getSetting('hatcherySort').value, Settings.getSetting('hatcherySortDirection').value, region))
