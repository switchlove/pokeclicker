window.addEventListener('load', () => {
    ...
    Game.prototype.start = function() {
        // Save.load() (called from `new Game()`, before start()) already ran Settings.fromJSON
        // against the saved settings, but our custom settings below didn't exist yet, so any
        // saved values for them were silently dropped. Track what's registered so far, then
        // re-apply the saved values for whatever we add below.
        const preAcsrqSettingNames = new Set(Settings.list.map((setting) => setting.name));

        // First, define all custom settings. This must be done before they are used.
        //#region ACSRQ Settings
        Settings.add(new BooleanSetting('hideNoti', 'Hide all notifications', false));
        Settings.add(new Setting('botCompleteNotifyDuration', 'Bot completion notification duration (seconds)', [], 5));
        ...
        Settings.add(new BooleanSetting('chestRush', 'Chest rush (prioritize chests)', false));
        Settings.add(new Setting('chestLimitOpts', 'Chest limit type', [
            new SettingOption('Count', 'chestLimitCount'),
            new SettingOption('Percent', 'chestLimitPercent'),
        ], 'chestLimitCount'));
        Settings.add(new RangeSetting('maxChestsPercent', 'Percent of chests to open', 0, 100, 5, 100));
        Settings.add(new Setting('chestQualityTiers', 'Chest qualities to open', [], ['common', 'rare', 'epic', 'legendary', 'mythic']));
        Settings.add(new Setting('dungeOpts', 'Dungeon bot stop options', [
            ...
            new SettingOption('Pokérus Resistance', 'dungOptPR'),
            new SettingOption('Region Shiny Count', 'dungOptRSC'),
        ], 'dungOptN'));
        ...
        Settings.add(new Setting('maxClearsGym', 'Maximum Clears', [], 1000));
        Settings.add(new Setting('minDT', 'Minimum DT to retain', [], 10000));
        Settings.add(new Setting('maxRegionShinies', 'Region shinies to stop at', [], 10));
        Settings.add(new Setting('maxLvl', 'Battle Frontier stop Options', [], 100));
        Settings.add(new Setting('maxTime', 'Time remaining to quit Battle Frontier at', [], 10));
        ...
        Settings.add(new Setting('safariOpts', 'Safari bot stop options', [
            ...
            new SettingOption('Level Check', 'safariOptLvl'),
        ], 'safariOptSC'));
        Settings.add(new Setting('maxSafariLvl', 'Safari Level to stop at', [], 40));
        Settings.add(new Setting('safariCatch', 'Safari bot catch options', [
            new SettingOption('None', 'safariCatchN'),
        ...
        Settings.add(new BooleanSetting('botstate.boostedRoute', 'Wander Bot', false));
        Settings.add(new Setting('botstate.plant', 'Farm Bot', [new SettingOption('N/A', 'N/A')], 'N/A'));
        Settings.add(new Setting('botstate.mutate', 'Mutate Bot', [new SettingOption('N/A', 'N/A')], 'N/A'));
        Settings.add(new BooleanSetting('botstate.mutateMulch', 'Mutate Bot: Use Mulch', true));
        //#endregion

        // Now that settings are defined, we can define the functions that use them.
        // These functions are now local to the Game.prototype.start override.
        
        //#region Info / Bot menu
        // Exposed on window because the generated data-bind strings below
        // reference `acsrqInfo.*` and are evaluated by Knockout in global scope.
        window.acsrqInfo = function () {
            ...
            Settings.getSetting('botstate.plant').options.push(
                new SettingOption('Unlock Plots', 'Unlock'),
                ...berries,
            ...
            const scriptingBody = [
                ...
                acsrqInfo.Checkbox('botstate.sr','TownList[\'Route 4 Pokémon Center\'].isUnlocked() || App.game.breeding.canAccess()'),
                acsrqInfo.Checkbox('botstate.boostedRoute'),
                acsrqInfo.PokeballPicker('ballBuyOpts'),
            ...
            ko.applyBindingsToDescendants(ko.contextFor(document.getElementById('acsrqContainer').parentNode), document.getElementById('acsrqContainer'));
        };
...
`;
    ...
    ${acsrqInfo.Row(
        ...
        `<select style="width: 100px;" onchange="Settings.setSettingByName(this.name, this.value)" data-bind="foreach: $data.options, attr: {name, id: 'select-' + $data.name}">
...
`;
        ...
        acsrqInfo.PokeballPicker = (setting) => `
    <tr data-bind="template: {data: Settings.getSetting('${setting}')}">
    ${acsrqInfo.Row(
        'Pokeball',
        `<span data-bind="foreach: $data.getValidOptions()">
            <img class="pokeball-small clickable" style="height:27px;"
                data-bind="
                    attr: {src: 'assets/images/pokeball/' + (GameConstants.Pokeball[$data.value] ?? 'None') + '.svg'},
                    css: {'pokeball-selected': $parent.observableValue() == $data.value},
                    click: function(){ Settings.setSettingByName('${setting}', $data.value); },
                    tooltip: {title: $data.text, trigger: 'hover', animation: false}
                "/>
        </span>`
    )}
    </tr>
`;
        ...
        const acsrqSettings = function () {
            const acsrq = [
                ...
                acsrqSettings.Section(
                    'UI', [
                        ...
                        acsrqSettings.Number('botCompleteNotifyDuration'),
            ...
            ko.applyBindingsToDescendants(ko.contextFor($('#settingsModal .tab-content')[0]), document.getElementById('settings-acsrq'));
            ...
            const acsrqScripting = [
                ...
                acsrqSettings.Section(
                    'Dungeons', [
                        ...
                        acsrqSettings.Template('BooleanSettingTemplate', 'chestRush', 'Settings.getSetting(\'chestCollect\').observableValue'),
                        acsrqSettings.Template('MultipleChoiceSettingTemplate', 'chestLimitOpts', 'Settings.getSetting(\'chestCollect\').observableValue'),
                        acsrqSettings.Number('maxChests', 'Settings.getSetting(\'chestCollect\').observableValue() && Settings.getSetting(\'chestLimitOpts\').observableValue() === \'chestLimitCount\''),
                        acsrqSettings.Range('maxChestsPercent', 'Settings.getSetting(\'chestCollect\').observableValue() && Settings.getSetting(\'chestLimitOpts\').observableValue() === \'chestLimitPercent\''),
                        acsrqSettings.MultiSelect('chestQualityTiers', [
                            { text: 'Common', value: 'common' },
                            { text: 'Rare', value: 'rare' },
                            { text: 'Epic', value: 'epic' },
                            { text: 'Legendary', value: 'legendary' },
                            { text: 'Mythic', value: 'mythic' },
                        ], 'Settings.getSetting(\'chestCollect\').observableValue'),
                        ...
                        acsrqSettings.Number('maxRegionShinies', 'Settings.getSetting(\'dungeOpts\').observableValue() === \'dungOptRSC\''),
                    ], false),
                acsrqSettings.Section(
                    'Gym', [
                        ...
                        acsrqSettings.Number('maxClearsGym', 'Settings.getSetting(\'gymOpts\').observableValue() === \'gymOptC\''),
                    ], false),
                acsrqSettings.Section(
                    'Mutate', [
                        acsrqSettings.Template('BooleanSettingTemplate', 'botstate.mutateMulch'),
                    ], false),
                acsrqSettings.Section(
                    'Pokeball', [
                ...
                acsrqSettings.Section('Safari', [
                    acsrqSettings.Template('MultipleChoiceSettingTemplate', 'safariOpts'),
                    acsrqSettings.Number('maxSafariLvl', 'Settings.getSetting(\'safariOpts\').observableValue() === \'safariOptLvl\''),
                ...
                acsrqSettings.Section(
                    'SoftReset', [
                        ...
                        acsrqSettings.Range('maxEggs', 'Settings.getSetting(\'srOpts\').observableValue() === \'egg\''),
                    ], false),
            ];
            ...
            ko.applyBindings(App.game, $('#settingsModal .nav-tabs')[0].lastElementChild);
            ...
            ko.applyBindingsToDescendants(ko.contextFor($('#settingsModal .tab-content')[0]), document.getElementById('settings-acsrq-script'));
        };
...
`;
        ...
        acsrqSettings.MultiSelect = (setting, options, visible = true) => {
            // The data-bind attribute is HTML-double-quote-delimited, so the options
            // literal must not contain double quotes - use single quotes instead of JSON.stringify.
            const optionsLiteral = JSON.stringify(options).replace(/"/g, '\'');
            return `
    <tr data-bind="visible: ${visible}">
        <td class="p-2" data-bind="text: Settings.getSetting('${setting}').displayName + ':'">setting name</td>
        <td class="p-0" data-bind="template: { name: 'multiSelectDropdownTemplate', data: {
            id: '${setting}',
            options: ${optionsLiteral},
            optionsText: 'text',
            optionsValue: 'value',
            bindingField: Settings.getSetting('${setting}').observableValue,
            buttonText: () => Settings.getSetting('${setting}').observableValue().map(v => (${optionsLiteral}.find(o => o.value === v) || {}).text).join(', ') || 'None',
            buttonClass: 'btn-secondary btn-sm',
            showSelectAll: true,
            selectAllClick: () => Settings.getSetting('${setting}').observableValue(${optionsLiteral}.map(o => o.value)),
        }}"></td>
    </tr>
`;
        };
        ...
        //#region Footer
        // Exposed on window because the generated data-bind strings below
        // reference `acsrqFooter.*` and are evaluated by Knockout in global scope.
        window.acsrqFooter = function () {
    ...
    `);
            ko.applyBindings(App.game, $('#battleContainer')[0].lastElementChild);
        };
    ...
    `);
        ...
        const originalEventStart = SpecialEvent.prototype.start;
        SpecialEvent.prototype.start = function() {
            originalEventStart.call(this);
        ...
        // Set up other subscriptions and overrides
        ...
        const originalDungeonInit = DungeonRunner.initializeDungeon;
        DungeonRunner.initializeDungeon = function(dungeon) {
            originalDungeonInit.call(this, dungeon);

            const setupDungeonShortcuts = () => {
                const chestSpan = $('#dungeonMap img[src*=\'chest\'] ~ span')[0];
                const encounterSpan = $('#dungeonMap img[src*=\'encounter\'] ~ span')[0];
                if (!chestSpan || !encounterSpan) {
                    // The #dungeonMap view hasn't been rendered yet, try again next frame
                    requestAnimationFrame(setupDungeonShortcuts);
                    return;
                }
                ...
                ko.applyBindingsToNode($('#dungeonMap img[src*=\'chest\'] ~ button')[0], {
                    ...
                    text: ko.pureComputed(() => {
                        const chestToOpen = Settings.getSetting('chestCollect').observableValue()
                            ? dungeonBot.chestLimit()
                            : 0;
                        return `${DungeonRunner.chestsOpened()}/${chestToOpen}/${DungeonRunner.map.totalChests()}`;
                    }),
                });

                $('#dungeonMap img[src*=\'encounter\'] ~ span')[0].insertAdjacentHTML('afterend', `
                    <img type="button" src="assets/images/dungeons/boss.svg" height="25px">
                `);
                ko.applyBindingsToNode($('#dungeonMap img[src*=\'encounter\'] ~ img')[0], {
                    visible: Settings.getSetting('botOptions').observableValue,
        ...
        Settings.getSetting('breedingSearchFilter').observableValue.subscribe((newValue) => {
            ...
            if (field && `/(${field.value})/i` != newValue) {
                field.value = newValue.slice(1,-1);
                if (typeof BreedingFilters !== 'undefined') {
        ...
        // Restore saved values for the custom settings registered above, since they weren't
        // present in Settings.list yet when Save.load() applied the saved settings.
        const savedSettings = JSON.parse(localStorage.getItem(`settings${Save.key}`) || localStorage.getItem('settings') || '{}');
        Settings.list
            .filter((setting) => !preAcsrqSettingNames.has(setting.name))
            .forEach((setting) => {
                if (Object.prototype.hasOwnProperty.call(savedSettings, setting.name)) {
                    setting.set(savedSettings[setting.name]);
                }
            });

        // Finally, call the original game start function to complete the loading process
        gameStart.call(this);
    };
});
